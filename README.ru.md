...
PicoOS сейчас находится в серии разработки `0.x`.

Версия `1.0.0` зарезервирована для первого полноценного релиза. Текущая версия `0.1.0` фиксирует RISC-V-only baseline после удаления экспериментального ARM64-кода из основной ветки.
