...
use crate::kernel::contract::{plan, BootContract};
mod bootstrap;
...
fn spawn_plan_workers() {
    match plan() {
        ...
        BootContract::Preempt => {
            let _ = spawn("worker-yield", crate::user::worker_yield_main, 0);
            uart::write_line("preempt image: U-mode yield loop");
        }
        BootContract::Ipc => {
            let _ = spawn("worker-ipc", crate::user::worker_ipc_parent, 0);
            uart::write_line("ipc image: parent spawns recv then send");
        }
        BootContract::UserText => {
            let _ = spawn("fetch-probe", crate::user::worker_kernel_fetch, 0);
            uart::write_line("user text image: jalr kernel .text");
        }
        BootContract::Default => {
            let _ = spawn("worker-yield", crate::user::worker_yield_main, 0);
        ...
        BootContract::Reap | BootContract::KernelFault => {}
    }
}
